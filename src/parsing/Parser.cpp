...
static const char CLOUD_POINT_PATH[2][18] = {
    "conf/ptcloud1.txt", "conf/ptcloud2.txt"};  // files name
static const char FILE_ERROR[] =
    "Unable to open file";  // error unable to open file
...
void Parser::parse(int cloud_nb) {
  ...
  for (int i = 0; i < MATRIX_SIZE;
       i++) {  // iterate trough the first line to get the matrix
    for (int j = 0; j < MATRIX_SIZE; j++) {
      file >> _matrix[cloud_nb][i][j];
    }
  }
  while (
      getline(file, line)) {  // iterate through all the files to get the points
    for (int i = 0; i < 2; i++) {
    ...
    point._points3D[3] =
        1;  // add manually 1 for homogenous coordianate and easier computation
    for (int i = 0; i < 3; i++) {
